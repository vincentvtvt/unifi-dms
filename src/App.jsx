...
@media(max-width:768px){
  ...
  .cta-flex{grid-template-columns:1fr!important;}
      ...
      <section style={{ width:"100%", background:"linear-gradient(160deg, "+UB.navy+" 0%, "+UB.blue+" 60%, "+UB.sky+" 100%)", padding:"90px 20px 60px", position:"relative", overflow:"hidden" }}>
        ...
        <div className="hero-flex" style={{ maxWidth:1100, margin:"0 auto", display:"flex", flexWrap:"wrap", gap:48, alignItems:"center" }}>
          <div className="hero-left fade-up" style={{ flex:"1 1 400px", maxWidth:540 }}>
            ...
            <div className="cta-flex" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16, maxWidth:440 }}>
              {[
                {id:"switch",label:"Switch & Save",sub:"FREE 6 months / FREE TV",color:UB.green,icon:Icons.trending},
                {id:"new",label:"New to Unifi",sub:"FREE 3 months on 300/500",color:"white",icon:Icons.zap},
                {id:"phone",label:"Free 5G Phone",sub:"Samsung / OPPO / Redmi",color:UB.sky,icon:Icons.phone},
                {id:"dms",label:"Digital Marketing",sub:"Ads from RM100/mo",color:UB.orange,icon:Icons.trending},
              ].map(o=>(
                <button key={o.id} onClick={()=>{setWizPath([o.id]);setWizTab(null);scr("solutions");}}
                  style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 14px", borderRadius:10, border:"1px solid rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.08)", color:"white", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.2s", textAlign:"left" }}
                  onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,0.18)";e.currentTarget.style.borderColor=o.color;}}
                  ...
                  <div style={{ width:32, height:32, borderRadius:8, background:"rgba(255,255,255,0.12)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{o.icon("white",16)}</div>
                  <div><div style={{ fontSize:13, fontWeight:700 }}>{o.label}</div><div style={{ fontSize:10, opacity:0.7 }}>{o.sub}</div></div>
                </button>
              ))}
            </div>
