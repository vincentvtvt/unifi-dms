...
export default function CloudStorage() {
  ...
  return (
    <>
      <PageHero icon={I.cloud(T.primary,28)} tag="Digital Solutions" title="Unifi Cloud Storage" subtitle="Secure cloud hosted in TM Tier-3 data centres. Enterprise security with local data residency." />
      <section style={{padding:"0 24px 60px",maxWidth:1200,margin:"0 auto"}}>
        ...
        <div style={{marginTop:24,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:16}}>
          <Card style={{padding:20}}><div style={{display:"flex",alignItems:"center",gap:10}}>{I.shield(T.primary,20)}<div><h4 style={{fontSize:15,fontWeight:700}}>Enterprise Security</h4><p style={{fontSize:13,color:T.muted,marginTop:4}}>Encryption at rest and in transit. TM certified data centres.</p></div></div></Card>
          <Card style={{padding:20}}><div style={{display:"flex",alignItems:"center",gap:10}}>{I.globe("#059669",20)}<div><h4 style={{fontSize:15,fontWeight:700}}>Local Data Residency</h4><p style={{fontSize:13,color:T.muted,marginTop:4}}>Data stays in Malaysia. Compliant with local regulations.</p></div></div></Card>
        </div>
      </section>
      <section style={{padding:"40px 24px 80px",textAlign:"center"}}>
        <WaBtn text="Get Cloud Storage" msg="I want to apply for Unifi Cloud Storage." />
      </section>
    </>
  );
}
